...
function seek_body_classes( $classes ) {
	...
	if ( ! is_singular() ) {
        ...
        if (seek_get_option('enable_header_fix_nav') == 1 ) {
        ...
        if( seek_get_option('homepage_layout_option') == "full-width"){
            $classes[] = "full-width";
        }else{
...
if( ! function_exists( 'seek_recommended_plugins' ) ) :
  ...
  function seek_recommended_plugins(){
      $seek_plugins = array(
        ...
        array(
            'name'      => esc_html__('Demo Import Kit','seek'),
            'slug'      => 'demo-import-kit',
            'required'  => false,
        ),
        array(
            'name'      => esc_html__('Themeinwp Import Companion','seek'),
